...
| YOLO-World 오픈보캡 검출 (프롬프트 → 짧은 파인튜닝) | 📝 준비됨 | [yolo_world_greenhouse_practice](notebooks/yolo_world_greenhouse_practice.ipynb) | 위 트랙에서 COCO 사전학습 검출기가 실패했던 바로 그 이미지에, 재학습 없이 `model.set_classes(["tomato"])` 프롬프트만으로 재도전. 이어서 같은 데이터(tomato 단일 클래스)로 5 epoch 짧은 파인튜닝까지 붙여 제로샷 대비 신뢰도 변화를 비교 |
