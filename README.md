...
| YOLO-World 오픈보캡 검출 (프롬프트 → 짧은 파인튜닝) | ✅ | [yolo_world_greenhouse_practice](notebooks/yolo_world_greenhouse_practice.ipynb) | COCO 사전학습 YOLO11n이 전부 오분류했던 이미지에 YOLO-World 제로샷 프롬프트로 재도전 → **검출 0개** (프롬프트 4종 다 시도해도 최대 1개). 같은 데이터로 5 epoch만 파인튜닝하니 **검출 7개, 평균 신뢰도 0.82, mAP50 0.931**(실측). 고정 클래스 검출기 < 오픈보캡 제로샷 < 짧은 파인튜닝, 세 단계 실측 비교 |
